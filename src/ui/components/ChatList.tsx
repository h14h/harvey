...
import { useAppState, useChats, useFocus, useSelectedChat } from "../store";

export interface ChatListProps {
	width?: number;
}

const _CHAT_ITEM_HEIGHT = 1;
		...
		>
			...
			{chats.length === 0 ? (
				...
				visibleChats.map((chat, index) => {
					const _actualIndex = chatListOffset + index;
					const isSelected = selectedChat?.id === chat.id;
					const cursor = isSelected ? "> " : "  ";

					return (
						<Box key={chat.id}>
							<Text color={isSelected ? "green" : isFocused ? "white" : "gray"} bold={isSelected}>
								{cursor}
								{chat.title}
							</Text>
						</Box>
			...
			{chats.length > maxVisibleChats && chatListOffset + maxVisibleChats < chats.length && (
				<Box>
					<Text color="gray" dimColor>
						▼ More below
					</Text>
