...
import { useContextUsage, useSendMessage, useToasts } from "./hooks";
...
function AppInner() {
	...
	const handleAction = useCallback(
		(action: string) => {
			...
			if (
				...
				(action === "sendMessage" || action === "enterInsertMode")
			) {
				return;
			}
	...
	useEffect(() => {
		...
		(globalThis as unknown as Record<string, unknown>).harveyKeyHandler = handler;
	 ...
	 */
	const handleSaveGlobalTone = useCallback(
		async (tone: string) => {
			setIsLoading(true);
			try {
	 ...
	 */
	const handleSaveChatAnchor = useCallback(
		async (anchorPrompt: string) => {
			setIsLoading(true);
			try {
				// Update the chat in the list
				const updatedChats = state.chats.map((chat) =>
					chat.id === state.selectedChatId ? { ...chat, anchorPrompt, updatedAt: Date.now() } : chat
				);
				dispatch({ type: "setChats", chats: updatedChats });
...
export function handleStdinKey(input: string, key: Record<string, unknown>): void {
	const handler = (globalThis as unknown as Record<string, unknown>).harveyKeyHandler as
		| KeyHandler
		| undefined;
