...
export function createMessageRepository(db: Database): MessageRepository {
	...
	return {
		create(data) {
			const now = Date.now();
			const result = insertMessage.run(data.chatId, data.role, data.content, data.turnNumber, now);
		...
		getLastNTurns(chatId, n) {
			...
			const rows = getMessagesFromTurnQuery.all(chatId, startTurn) as MessageRow[];
		...
		getMessagesSinceTurn(chatId, turn) {
			const rows = getMessagesSinceTurnQuery.all(chatId, turn) as MessageRow[];
			return rows.map((message) => toMessage(message));
		},
